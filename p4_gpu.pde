...
void draw() {
  ...
  int yStart = 50;
  int yEnd = 300;
  float deltaX = (xEnd - xStart) / (divisions);
  float deltaY = (yEnd - yStart) / (divisions);
  float deltaU = 1 / divisions;
  float deltaV = 1 / divisions;
  for (int yDivide = 0; yDivide < divisions; yDivide++) {
    for (int xDivide = 0; xDivide < divisions; xDivide++) {
      beginShape();
      texture(ship_texture);
      float x1 = xStart + (xDivide) * deltaX;
      float y1 = yStart + (yDivide) * deltaY;
      ...
      float u1 = deltaU * xDivide;
      float v1 = deltaV * yDivide;
      ...
      vertex(x1, y1, -300, u1, v1);
      vertex(x2, y2, -300, u2, v2);
      vertex(x3, y3, -300, u3, v3);
      vertex(x4, y4, -300, u4, v4);
      endShape();
    }
  }
